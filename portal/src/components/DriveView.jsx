  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-8 pb-12">
      ...
      {loading ? (
        ...
        <p className="loading-card mx-auto block my-6">Searching across {root.name}...</p>
      ) : isSearching && searchError ? (
        <p className="text-wcs-red text-sm py-4 text-center">{searchError}</p>
      ) : !isSearching && files.length === 0 ? (
        <p className="empty-card mx-auto block my-6">This folder is empty.</p>
      ) : visibleFiles.length === 0 ? (
        <p className="empty-card mx-auto block my-6">{isSearching ? 'No matches.' : 'No files match your filter.'}</p>
      ) : viewMode === 'grid' ? (
        <>
