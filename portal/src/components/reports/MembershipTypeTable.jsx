// Shared horizontal-bar breakdown, now used only by Cancels (by Membership
// Type).
//
// Always drawn in full. It could start hidden behind a pill for Club Health,
// which showed two of these; both are gone, and Cancels never used the pill,
// so the toggle was a branch nothing could reach.
//
// flush = render without the bordered card chrome (for use inside a
// ReportBlock).
export default function MembershipTypeTable({ title, rows, flush = false }) {
  const list = rows || []
