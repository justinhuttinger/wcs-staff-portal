...
export default function AdminPrintDevicesTab() {
  ...
  if (loading) {
    return <div className="bg-surface border border-border rounded-xl p-4 text-text-muted">Loading devices...</div>
  }
  if (!devices.length) {
    return (
      <div className="bg-surface border border-border rounded-xl p-4 text-text-muted">
        No devices have checked in yet. Install the launcher at a gym and it will appear here.
      </div>
    )
  }
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-4 space-y-4">
      {msg && <div className="text-sm text-text-primary">{msg}</div>}
      {devices.map(d => {
        ...
        return (
          <div key={d.install_id} className="bg-bg border border-border rounded-lg p-4">
            <div className="flex items-center justify-between">
              ...
              <span className={'text-xs px-2 py-0.5 rounded ' + (online ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-text-muted')}>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              ...
              <button
                className="text-sm px-3 py-1 rounded bg-wcs-red text-white hover:bg-wcs-red-hover disabled:opacity-50"
