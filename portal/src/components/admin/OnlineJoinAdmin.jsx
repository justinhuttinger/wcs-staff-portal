...
const TABS = [
  ...
  { key: 'plans',     label: 'Plans',     desc: 'Per-location plans + ABC IDs (with Pull from ABC)' },
  { key: 'age-rules', label: 'Age Rules', desc: 'Named age ranges with live preview' },
  { key: 'copy',      label: 'Copy',      desc: 'Editable global strings' },
  { key: 'signups',   label: 'Signups',   desc: 'Read-only signup log, filterable' },
]
  ...
  return (
    <div className="space-y-4">
      <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-2 text-xs text-yellow-800">
        <span className="font-semibold">Experimental — Online Join admin.</span> All 5 admin sub-pages live. Public widget + signup API are still TBD (Phase 4+).
      </div>
