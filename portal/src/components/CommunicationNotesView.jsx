  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-8 pb-12">
      ...
      <div className="bg-surface/95 backdrop-blur-sm rounded-xl border border-border p-5 mb-6">
        <h2 className="text-xl font-black text-text-muted tracking-[-0.5px]">Communication Notes</h2>
      </div>
      ...
      {!isLeadPlus && <div className="bg-surface border border-border rounded-xl mb-6 overflow-hidden">
        ...
        >
          <span className="text-sm font-semibold text-text-muted">Submit a Note</span>
          <svg
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
          ...
          <form onSubmit={handleSubmit} className="px-5 pb-5 space-y-3 border-t border-border pt-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm rounded-lg border border-border bg-bg text-text-muted placeholder:text-text-muted/50 focus:outline-none focus:border-wcs-red"
                required
              />
            </div>
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 text-sm rounded-lg border border-border bg-bg text-text-muted focus:outline-none focus:border-wcs-red"
                required
              >
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-3 py-2 text-sm rounded-lg border border-border bg-bg text-text-muted placeholder:text-text-muted/50 focus:outline-none focus:border-wcs-red resize-none"
                required
              />
            </div>
          ...
          <div className="bg-surface border border-border rounded-xl mb-5">
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center gap-2 px-4 py-3 text-sm font-medium transition-colors ${
                      active ? 'text-text-muted' : 'text-text-muted hover:text-text-muted'
                    }`}
                  >
                    <span className={`w-2 h-2 rounded-full ${dotColor}`} />
                    <span>{STATUS_LABELS[s]}</span>
                    <span className={`text-xs ${active ? 'text-text-muted font-semibold' : 'text-text-muted'}`}>
                      {statusCounts[s] || 0}
                    </span>
            ...
            <div className="flex items-center gap-3 px-4 py-2.5 flex-wrap">
              <select
                ...
                className="px-3 py-1.5 text-xs rounded-lg border border-border bg-bg text-text-muted focus:outline-none focus:border-wcs-red"
              >
              ...
              {statusFilter === 'completed' && (
                <div className="flex items-center gap-1.5 ml-auto">
                  <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="w-[130px] px-2 py-1.5 text-xs rounded-lg border border-border bg-bg text-text-muted focus:outline-none focus:border-wcs-red" />
                  <span className="text-xs text-text-muted">to</span>
                  <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="w-[130px] px-2 py-1.5 text-xs rounded-lg border border-border bg-bg text-text-muted focus:outline-none focus:border-wcs-red" />
                  <button
                    onClick={() => { setDateFrom(''); setDateTo('') }}
                    className="px-2.5 py-1.5 text-xs text-text-muted hover:text-text-muted"
                  >
                    Clear
                  </button>
                </div>
                      ...
                      <div className="flex items-start gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-sm font-semibold text-text-muted truncate">{note.title}</span>
                        ...
                        <div>
                            ...
                            <div className="space-y-2 mb-3">
                              {comments.map(c => (
                                <div key={c.id} className="bg-bg rounded-lg px-3 py-2">
                                  <div className="flex items-center gap-2 mb-1">
                                    <span className="text-xs font-semibold text-text-muted">{c.author_name || 'Unknown'}</span>
                          ...
                          <div className="flex gap-2">
                            <textarea
                              ...
                              rows={2}
                              className="flex-1 px-3 py-2 text-sm rounded-lg border border-border bg-bg text-text-muted placeholder:text-text-muted/50 focus:outline-none focus:border-wcs-red resize-none"
                            />
                            <button
