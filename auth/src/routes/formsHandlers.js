...
const { canAccessForm } = require('../services/formsPermissions')
const { validateSchema, makeSlug, INPUT_TYPES, DISPLAY_TYPES } = require('../services/formsSchema')
const { normalizeQuizSettings } = require('../services/quizSchema')
const formsSheets = require('../services/formsSheets')
const formsAudit = require('../services/formsAudit')

// Shared handlers for Forms and Quiz Funnels. buildFormsRouter() is mounted at
// /forms (kind='form') and /quizzes (kind='quiz'); each mount only ever sees
// rows of its own kind, and each has its own module gate.
...
// Legacy rows written before the kind column read as forms.
...
function buildFormsRouter({ kind, gate }) {
  ...
  // The whole module is gated (admin, or an explicit RBAC grant); per-form
  ...
  router.get('/', async (req, res) => {
    try {
      const { data: forms, error } = await supabaseAdmin.from('forms')
        .select('*').eq('kind', kind).order('updated_at', { ascending: false })
      if (error) throw error
      const all = forms || []
      let visible
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      // Quizzes have no owning club: they run at clubs via quiz_clubs.
      if (kind === 'quiz') {
        const row = {
      ...
      const row = {
        ...
        description: description || null, owner_id: req.staff.id, location_id: loc, kind: 'form',
      }
      const { data, error } = await supabaseAdmin.from('forms').insert(row).select('*').single()
      if (error) throw error
  ...
  router.get('/:id', async (req, res) => {
    try {
      const { form, shares, access } = await load(req, req.params.id)
  ...
  router.patch('/:id', async (req, res) => {
    try {
      ...
      if (settings !== undefined) {
        ...
        // (normalizeQuizSettings already merges section by section).
      ...
      const fresh = await load(req, form.id)
  ...
  router.get('/:id/submissions', async (req, res) => {
    try {
      ...
      let q = supabaseAdmin.from('form_submissions')
        .select('id, data, submitted_at, synced_to_sheet, sync_error, location_id, webhook_status, webhook_attempts, webhook_error', { count: 'exact' })
        .eq('form_id', form.id)
