...
## Task 3.5: Manual test harness script

**Files:**
- Create: `ghl-sync/scripts/test-referral.js`

A manual CLI to exercise the real ABC + GHL paths against a designated comp account
(safe + reversible), independent of the sync cycle. Also confirms the real
`invoiceadjustment` success-response shape. Not used in production.
...
- [ ] **Step 2: Verify it parses + prints usage**

Run: `node ghl-sync/scripts/test-referral.js`
Expected: prints the usage block and exits non-zero (no crash, modules resolve).

- [ ] **Step 3: Commit**

```bash
git add ghl-sync/scripts/test-referral.js
git commit -m "feat(ghl-sync): manual referral test harness (inspect/zero/restore/tag)"
```

**Manual validation (Justin, with the comp account — not an automated step):**
`inspect` the comp member to confirm invoices + the chosen DUES invoice; `zero` it and
note the success-response body (confirms `adjustInvoice` success detection); `restore`
it to the original amount; `tag` a test GHL contact to confirm the SMS workflow fires.
